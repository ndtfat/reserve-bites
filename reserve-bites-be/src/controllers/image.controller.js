...
export default {
  async uploadSingleImage(req, res) {
    try {
      ...
      const image = await Image.findOne({ name: `${originalname} (${new Date().toDateString()})` });
      ...
      const newImage = new Image({ url: dowloadUrl, name: `${originalname} (${new Date().toDateString()})` });
      await newImage.save();

      return res.status(200).send(newImage.toObject());
    } catch (e) {
      console.log(e);
      res.status(500).send({ message: "Something wrong with upoad main image" });
    }
  },
};
