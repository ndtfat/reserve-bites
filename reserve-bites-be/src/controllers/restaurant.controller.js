...
export default {
  ...
  async putUpdateRestaurant(req, res) {
    try {
      ...
      let updatedRestaurant = await Restaurant
        .findOneAndUpdate({ ownerId: owner.id }, restaurant, { new: true })
        .populate("ownerId")
  ...
  async geLocalRestaurants(req, res) {
    try {
      ...
      if (address) {
        ...
        }).limit(8).populate("mainImage", "url name");;
  ...
  async getReviews(req, res) {
    try {
      ...
      const reviews = await Review.find({ rid })
